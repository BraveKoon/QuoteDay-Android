...
컴파일과 단위 테스트가 모두 통과해도 앱은 첫 프레임에서 죽을 수 있다. 자원을
못 읽거나(`:core` 는 JSON 을 `getResourceAsStream` 으로 읽는데, APK 안에서는
사정이 다르다), 시작할 때 거는 알람이 예외를 던지거나, 화면 하나가 빈 목록에서
넘어지거나. 어느 것도 앞의 검사에는 걸리지 않는다.

그래서 CI 에 에뮬레이터 잡을 둔다. 앱을 켜서 다섯 탭을 모두 눌러 보고, 설정
화면에 데이터 개수가 뜨는지 본다 — 그 줄이 뜬다는 것은 JSON 이 APK 안에서
읽혔다는 뜻이다.
...
- [x] 위젯 — 홈 화면의 오늘의 명언
- [x] CI — `:core` 테스트, `:app` 디버그 APK, 릴리스 빌드, 에뮬레이터에서 켜 보기
- [x] 릴리스 워크플로 — 서명된 AAB
...
개인정보처리방침은 `docs/PRIVACY.md` 에 있다. iOS 판을 그대로 쓰면 안 된다 —
**하트가 어디에 쌓이는지**가 두 플랫폼에서 다르다. iOS 는 iCloud 를 거치고,
안드로이드는 기기 안에만 쌓는다. Play Console 에는 이 문서를 볼 수 있는 URL 이
필요하므로, GitHub 의 파일 주소를 그대로 넣으면 된다.
